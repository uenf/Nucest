...
Dado /^que eu tenho uma instituição$/ do
  @instituicao = FactoryGirl.create :instituicao, :nome => "UENF1",
                                              :cnpj => "69.103.604/0001-60"
end

Dado /^que eu tenho uma instituição com nome "([^"]*)"$/ do |nome|
  @instituicao_com_nome = FactoryGirl.create :instituicao, :nome => nome
end

Dado /^que eu tenho uma instituição com CNPJ "([^"]*)"$/ do |cnpj|
  @instituicao_com_nome = FactoryGirl.create :instituicao, :nome => "UENF2",
                                                       :cnpj => cnpj
end

Dado /^que eu tenho uma instituição com razão social "([^"]*)"$/ do |razao_social|
  @instituicao_com_nome = FactoryGirl.create :instituicao, :nome => "UENF3",
